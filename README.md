...
面向《异环》的 Windows 桌面自动钓鱼工具：自动抛竿、自动上钩、自动溜鱼、自动结算、自动记录战绩。
  ...
  <p>
    ...
    <img alt="Version" src="https://img.shields.io/badge/version-1.2-63E4E4?style=flat-square">
  </p>
</div>
...
1.2 是一次稳定性和可用性修复版本，重点解决 1.1 中的分辨率适配、溜鱼识别、失败恢复、记录统计、更新链路和用户交互问题。
...
完整发布说明见 [release_notes_v1.2.md](release_notes_v1.2.md)。

## 下载与运行

推荐使用 GitHub Release 中的压缩包，不需要自己配置 Python 环境。

1. 下载 `YHoAutoFish-v1.2-windows.zip`。
...
1.2 的默认参数已按当前游戏 A/D 移动速度调校。多数用户不需要修改。
...
受保护的数据包括 `config.json`、`records.json`、`records.db`、`logs/`、`screenshots/`、`captures/` 等。
...
| 问题 | 解决办法 |
| --- | --- |
| 程序启动要求管理员 | 这是 1.2 的强制要求，点击 UAC 窗口“是” |
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\build_release.ps1 -SkipInstall -NotesFile .\release_notes_v1.2.md
```

构建完成后会生成：

- `release/YHoAutoFish-v1.2-windows.zip`
- `release/latest.json`

发布 GitHub Release 时，上传上述两个文件，并将 `release_notes_v1.2.md` 的内容粘贴到 Release 正文。
